...
describe('verifyTurnstile', () => {
  describe('token validation', () => {
    ...
    test('accepts token exactly 2048 characters', async () => {
      server.use(http.post(TURNSTILE_URL, () => HttpResponse.json({ success: true, hostname: 'eve0415.net' })));
  ...
  describe('success cases', () => {
    ...
    test('returns success when no hostname is provided in response', async () => {
      server.use(http.post(TURNSTILE_URL, () => HttpResponse.json({ success: true })));
  ...
  describe('error code mapping', () => {
    ...
    test('returns generic error when no error codes provided', async () => {
      server.use(http.post(TURNSTILE_URL, () => HttpResponse.json({ success: false })));
