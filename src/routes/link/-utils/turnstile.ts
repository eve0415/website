...
export async function verifyTurnstile(token: string, secretKey: string, remoteIp?: string): Promise<TurnstileResult> {
  ...
  try {
    ...
    if (!result.success) {
      ...
      if (errorCodes.includes('timeout-or-duplicate')) {
        return { success: false, error: '認証がタイムアウトしました。ページを再読み込みしてください。' };
      }
      if (errorCodes.includes('invalid-input-response')) {
        return { success: false, error: '認証トークンが無効です。もう一度お試しください。' };
      }
