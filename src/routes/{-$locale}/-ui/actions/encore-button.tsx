...
export const EncoreButton: FC<EncoreButtonProps> = ({ dot = true, className, children, ...rest }) => (
  <button type='button' className={cn(ROOT, className)} {...rest}>
    ...
    <span className='relative z-[1] inline-flex items-center gap-[9px] transition-[color] delay-[0.12s] duration-[0.22s] ease-[ease] group-hover:text-[var(--ink-on-accent)]'>
      {dot ? (
        <span className='h-[6px] w-[6px] [animation:evGlowPulse_2.8s_ease-in-out_infinite_alternate] rounded-[50%] bg-current' aria-hidden='true' />
      ) : null}
      {children}
    </span>
  </button>
