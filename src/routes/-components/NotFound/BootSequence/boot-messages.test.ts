...
describe('boot-messages', () => {
  ...
  describe('bOOT_MESSAGES', () => {
    ...
    it('contains expected major sections', () => {
      const rootIds = BOOT_MESSAGES.map(m => m.id);
    ...
    it('hydration section ends with error messages', () => {
      const hydrate = BOOT_MESSAGES.find(m => m.id === 'hydrate');
