...
describe('ErrorCascade', () => {
  describe('visibility thresholds', () => {
    ...
    test.each([
      ...
      { progress: 1.0, expectedVisible: 8, desc: '8 errors at progress 1.0' },
    ])('$desc (progress=$progress)', async ({ progress, expectedVisible }) => {
      await render(<ErrorCascade progress={progress} enabled={true} />);
      ...
      expect(errorElements.length).toBe(expectedVisible);
    });
    ...
    test('exponential curve: progress^0.7 formula', () => {
      ...
      const effectiveAt05 = Math.pow(0.5, 0.7);
      ...
      const effectiveAt1 = Math.pow(1.0, 0.7);
  ...
  describe('disabled state', () => {
    ...
    test('returns null when progress < 0.05', async () => {
      const { container } = await render(<ErrorCascade progress={0.03} enabled={true} />);

      expect(container.innerHTML).toBe('');
    });

    test('returns null when progress is 0', async () => {
      const { container } = await render(<ErrorCascade progress={0} enabled={true} />);
  ...
  describe('error content', () => {
    test('displays first error message at threshold', async () => {
      await render(<ErrorCascade progress={0.1} enabled={true} />);
    ...
    test('displays multiple error types as progress increases', async () => {
      await render(<ErrorCascade progress={0.5} enabled={true} />);
    ...
    test('displays catastrophic errors at high progress', async () => {
      await render(<ErrorCascade progress={1.0} enabled={true} />);
  ...
  describe('error ordering', () => {
    test('errors appear in threshold order', async () => {
      ...
      expect(texts[texts.length - 1]).toContain('Kernel panic');
    });
  });
  ...
  describe('animation delays', () => {
    test('error boxes have staggered animation delays', async () => {
      ...
      for (const box of errorBoxes) {
        ...
        if (style) {
          const ms = Number.parseInt(style);
          delays.push(ms);
        }
      }
