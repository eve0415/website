...
describe('AftermathScene', () => {
  ...
  describe('error selection', () => {
    test.each([
      ...
      await render(<TestWrapper visible={true} />);
  ...
  describe('visibility', () => {
    test('returns null when not visible', async () => {
      ...
      for (const marker of markers) {
        await expect.element(page.getByText(marker)).not.toBeInTheDocument();
      }
    });
  ...
  describe('consistency', () => {
    test('error selection is deterministic based on Date.now seed', async () => {
      ...
      // First render
      const { unmount } = await render(<TestWrapper visible={true} />);
