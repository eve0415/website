...
Inside `-/`, nest by what the thing is, as deep as it needs. A component with children of its own becomes `contact-form/index.tsx` plus siblings; a family of related components becomes `-/routed-links/`. **`index.tsx` is always the component itself, never a re-export barrel** — import from the module that defines the thing.
